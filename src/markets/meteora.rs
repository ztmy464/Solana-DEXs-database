...
impl MeteoraDEX {
    pub fn new(mut dex: Dex) -> Self {
        ...
        for pool in json_value.clone() {
            // 将 pool 这个结构体序列化成了二进制数据，存在 serialized_data 中
            let mut serialized_data: Vec<u8> = Vec::new();
            // &mut X ：是 可变引用 表示借用 X 并允许修改其内容
            // result.unwrap() ：从 Option / Result 取值
            ...
            // 为每个交易对收集所有可用市场池
            // "USDC-SOL" => [Raydium池, Meteora池, Orca池...]
